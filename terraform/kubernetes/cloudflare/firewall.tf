...
#   expression  = "(http.host eq \"plex.${var.cloudflare_domain}\" and http.request.uri.path contains \"/:/eventsource/notifications\")"
# }
...
resource "cloudflare_filter" "domain_github_flux_webhook" {
  ...
  expression  = "(http.host eq \"flux-webhook.${var.cloudflare_domain}\" and ip.geoip.asnum eq 36459)"
}
